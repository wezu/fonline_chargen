...
class UI(DirectObject):
    ...
    '''
    def __init__(self, font_size=13, big_font_size=24):
        #load fonts
        self.font_size=font_size
        self.font = loader.load_font('font/Roboto-Bold.ttf')
        ...
        self.mono_font= loader.load_font('font/iosevka-medium.ttf')
        self.font.set_space_advance(0.5)
        self.font.set_pixels_per_unit(font_size)
        self.font.set_minfilter(SamplerState.FT_nearest )
        self.font.set_magfilter(SamplerState.FT_nearest )
        ...
        self.big_font=self.font.make_copy()
    ...
    def software_cursor(self):
        self.cursor=self.tiles['mouse_icon'].copy_to(NodePath('icon'))
        self.cursor.reparent_to(pixel2d)
        self.cursor.set_pos(9, 0, -17)
        self.cursor.flatten_light()
        taskMgr.add(self.track_mouse, 'track_mouse_tsk', sort=-50)

    def track_mouse(self, task):
        if base.mouseWatcherNode.hasMouse():
            mpos = base.mouseWatcherNode.getMouse()
            self.cursor_pos=Point3(mpos.getX() ,0, mpos.getY())
            self.cursor.set_pos(pixel2d.getRelativePoint(render2d, self.cursor_pos))
        return task.cont
    ...
    def sort_buttons(self, canvas_name, sort_key):
        ...
        for button in self.elements[canvas_name].get_children():
            if button.has_python_tag('sort_dict'):
        ...
        for frame in data['frame']:
            ...
            if 'img' in frame:
                for img in frame['img']:
                    ...
                    if 'name' in img:
                        self.elements[img['name']]=self.img(**img)
                    else:
                        self.img(**img)
            #make inputs
            if 'input' in frame:
            ...
            #make icon
            if 'icon' in frame:
                for icon in frame['icon']:
                    if not 'parent' in icon:
                        icon['parent']=frame['name']
                    self.icon(**icon)
    ...
    def highlight(self, button=None, on=True, name=None):
        ...
        if on:
            ...
            button['text_fg']=(0.6,0.65,0.6,1.0)
    ...
    def img(self, texture, parent, pos, tex_scale=(1,1), uv_offset=(0,0), scale=1, hidden=False, name=None):
        ...
        #color_attrib = ColorBlendAttrib.make(ColorBlendAttrib.M_add, ColorBlendAttrib.O_incoming_color, ColorBlendAttrib.O_one )
        #node.setAttrib(color_attrib)
        if hidden:
            node.hide()
        return node

    def text(self, txt, pos, parent, center=True, big_font=True, mono_font=False, wordwrap=None, name=None, hidden=False,):
    ...
    def button(self, txt, cmd, width, pos, parent, txt_fg=(0.6,0.65,0.6,1.0), center=True, style='', hidden=False, sort_dict={}, name=None, mono_font=False):
        '''Creates a button
        '''
        if parent in self.elements:
            parent = self.elements[parent]
    ...
    def icon(self, cmd, pos, parent, style, size=32, name=None):
        ''' Creates a icon-button, a 32x32 button with an image'''
        if parent in self.elements:
            parent = self.elements[parent]
        geom=self.tiles[style+'_icon'].copy_to(NodePath('icon'))
        root=DirectFrame(frameColor=(1,0,0, 0.0),
                         frameSize=None,#_rec2d(32, 32),
                         state=DGG.NORMAL,
                         geom=geom,
                         geom_pos=_pos2d(size//2, size//2),
                         pad=(-2, -2),
                         suppressMouse = True,
                         parent=parent)
        root.set_pos(_pos2d(*pos))
    ...
    def slider(self, value, cmd, width, pos, parent, value_range=(0,1), name=None, lazy=False):
        ''' Creates a horizontal slider.
        Use 'value' in the cmd text to pass the value of the slider to the callback eg.
        cmd="print('This slider is set to:', value)"
        if lazy is True, the cmd is executed only on mouse click/release
        '''
        if parent in self.elements:
            parent = self.elements[parent]
        max_width = int(width)//32
    ...
    def logo(self, show=False):
        ''' Function to reliably show/hide the game logo'''
        self._update_gui_nodes()
        base.graphicsEngine.renderFrame()
        base.graphicsEngine.renderFrame()
        base.graphicsEngine.renderFrame()
        is_hidden=False
        if 'logo' in self.elements:
            is_hidden=self.elements['logo'].is_hidden()
            self.elements['logo'].remove_node()
        if show:
            is_hidden=False
        x=base.win.get_x_size()
        y=base.win.get_y_size()
        img=loader.load_texture('ui/logo_small.png')
        scale=(128, 1, 64)
        ...
        self.elements['logo']= OnscreenImage(image = img, scale=scale, pos = (0, 0, -50), parent=self.elements['top'])
        if is_hidden:
            self.elements['logo'].hide()
    ...
    def on_window_resize(self):
        #self.logo()
        self._update_gui_nodes() #self.logo() calls this
